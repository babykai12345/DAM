...
def main():
    np.random.seed(1)
    gene_df = pd.read_csv('train_x_gene_noname_re.csv', header=None)
    gene = gene_df.values
    ...
    H10_df.to_csv('H_gene.csv', header=None, index=False)
    U_df = pd.DataFrame(U_)
    U_df.to_csv('U_gene.csv', header=None, index=False)
