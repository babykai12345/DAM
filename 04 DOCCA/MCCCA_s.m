function [u, v, obj] = MCCCA_s(X, Y, opts)
...
while (i<max_Iter && (tu>tol || tv>tol)) % default 100 times of iteration
    ...
    obj(end+1) = -u'*X'*Y*v+lambda1*v'*Y'*LY*Y*v+lambda2*u'*X'*LX*X*u+alpha2*(u'*XX*u-1)+alpha1*(v'*YY*v-1);
end
end